    ...
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center h-16">
            <a href="/" class="flex items-center gap-2 group">
                <div class="w-11 h-11 bg-gradient-to-br from-orange-500 via-red-500 to-purple-600 rounded-2xl rotate-3 flex items-center justify-center text-white font-extrabold text-2xl shadow-lg shadow-orange-200 group-hover:rotate-12 transition transform duration-300">V</div>
            ...
            <div class="hidden md:flex space-x-1 items-center bg-gray-100/50 p-1 rounded-full">
                ...
                <a href="/haberler/" class="px-4 py-2 rounded-full text-gray-700 font-bold transition hover:bg-white hover:text-orange-600 hover:shadow-sm">Haberler</a>
                <a href="/#oyuncular" class="px-4 py-2 rounded-full text-gray-700 font-bold transition hover:bg-white hover:text-orange-600 hover:shadow-sm">Oyuncular</a>
            </div>

            <div class="flex items-center gap-3">
                {% if user.is_authenticated %}
                    {% if user.menajer %}
                        <a href="/menajer-panel/" class="bg-gradient-to-r from-indigo-900 to-purple-800 text-white px-4 py-2 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition flex items-center gap-2">💼 Panel</a>
                    {% elif user.sporcu %}
                        <a href="/profil-duzenle/" class="bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition flex items-center gap-2">✏️ Profil</a>
                    {% endif %}
                    <a href="/cikis/" class="text-red-500 hover:text-red-700 font-bold text-sm">Çıkış</a>
                {% else %}
                    <a href="/giris/" class="text-indigo-900 font-bold hover:text-orange-500 transition">Giriş</a>
                    <a href="/kayit/" class="bg-indigo-900 text-white px-5 py-2 rounded-full hover:bg-indigo-800 font-bold text-sm transition shadow">Kayıt Ol</a>
                {% endif %}
            </div>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4 py-16">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
            <div class="col-span-1 md:col-span-2">
                ...
                <p class="text-sm leading-relaxed mb-6 opacity-80">Türkiye'nin en gelişmiş voleybol veri platformu. Oyuncular, transferler, canlı skorlar ve daha fazlası.</p>
...
<body class="bg-gray-50 flex flex-col min-h-screen">
    ...
    <main class="flex-grow container mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            ...
            <div class="p-4 rounded-lg bg-green-100 text-green-800 border-l-4 border-green-500 shadow-sm">✅ {{ message }}</div>
            {% endfor %}
        </div>
        {% endif %}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-12">
                        ...
                        {% for item in mansetler %}
                            {% if item.is_ad %}
                                <div class="swiper-slide relative bg-gray-900">
                                    <img src="{{ item.image }}" alt="Reklam" class="opacity-60">
                            ...
                            {% else %}
                                <div class="swiper-slide relative">
                                    ...
                                    <div class="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/80 to-transparent p-8 md:p-12 text-left">
                                        ...
                                        <a href="/haber/{{ item.id }}/" class="text-white border-b-2 border-orange-500 pb-1 hover:text-orange-400 font-bold">Haberi Oku →</a>
                                    </div>
                                </div>
                            {% endif %}
                        {% endfor %}
            ...
            <div class="hidden lg:block lg:col-span-1 h-[450px]">
                <div class="bg-gray-200 h-full rounded-[2rem] flex items-center justify-center relative overflow-hidden shadow-xl border-4 border-white">
                    <img src="https://via.placeholder.com/400x600/222/FFF?text=REKLAM" class="w-full h-full object-cover opacity-80">
                    <div class="absolute top-4 right-4 bg-black/30 text-white text-[10px] px-2 rounded">REKLAM</div>
                </div>
            </div>
        </div>
            ...
            <div class="lg:col-span-3" id="oyuncular">
                <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
                    ...
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="flex gap-2"><input type="number" name="min_boy" placeholder="Min Boy" class="border p-3 rounded-xl w-1/2 bg-gray-50"><input type="number" name="max_boy" placeholder="Max" class="border p-3 rounded-xl w-1/2 bg-gray-50"></div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-[2rem] shadow-lg overflow-hidden border border-indigo-50">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tr class="border-b hover:bg-indigo-50 transition"><td class="p-4 font-medium">{{ forloop.counter }}. {{ sira.kulup.isim }}</td><td class="p-4 font-bold text-right text-indigo-900">{{ sira.puan|floatformat:0 }}</td></tr>
                        {% endfor %}
                    </table>
                </div>
            </div>
...
""" + INDEX_HTML_BODY.replace('__NAVBAR__', NAVBAR_HTML).replace('__FOOTER__', FOOTER_HTML)
...
<body class="bg-gray-100 flex flex-col min-h-screen">
    ...
    <main class="flex-grow container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-[2rem] shadow-xl overflow-hidden flex flex-col md:flex-row mb-8 border border-gray-100">
            <div class="md:w-1/3 bg-gray-200 relative min-h-[450px]">
                ...
                <div class="absolute bottom-4 right-4 z-10">
                    <form method="POST">{% csrf_token %}<button name="begen" class="bg-white/90 backdrop-blur text-red-500 px-6 py-3 rounded-full shadow-lg font-bold hover:scale-105 transition flex items-center gap-2 text-lg">❤️ {{ sporcu.begeni_sayisi }}</button></form>
                </div>
            </div>
            <div class="p-8 md:w-2/3">
                <h1 class="text-5xl font-extrabold text-gray-900 mb-4">{{ sporcu.isim }}</h1>
                <div class="flex gap-3 mb-8">
                    <span class="bg-indigo-100 text-indigo-800 px-4 py-1 rounded-full font-bold">{{ sporcu.kulup.isim }}</span>
                    <span class="bg-orange-100 text-orange-800 px-4 py-1 rounded-full font-bold">{{ sporcu.get_mevki_display }}</span>
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-600 mb-8">
                    <thead class="bg-gray-50 text-gray-500 uppercase text-xs"><tr><th class="p-3">Sezon</th><th class="p-3">Eski</th><th class="p-3">Yeni</th></tr></thead>
                    <tbody>
                        {% for t in sporcu.transferler.all %}
                        <tr class="border-b"><td class="p-3 font-bold text-indigo-900">{{ t.sezon }}</td><td class="p-3">{{ t.eski_kulup.isim|default:"-" }}</td><td class="p-3 font-bold">{{ t.yeni_kulup.isim|default:"-" }}</td></tr>
                        {% endfor %}
                    </tbody>
                </table>
                
                {% if sporcu.video_linki %}
                <a href="{{ sporcu.video_linki }}" target="_blank" class="block w-full bg-red-600 text-white text-center py-4 rounded-xl font-bold hover:bg-red-700 transition">▶ Tanıtım Videosunu İzle</a>
                {% endif %}
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto mt-16">
            ...
            {% if user.is_authenticated %}
            <form method="POST" class="mb-10 bg-white p-6 rounded-[2rem] shadow-lg border border-indigo-50">
                {% csrf_token %}
                <input type="hidden" name="yorum_yaz" value="1">
                <textarea name="metin" rows="3" class="w-full border-2 border-gray-100 p-4 rounded-xl focus:outline-none focus:border-orange-400 transition resize-none" placeholder="Oyuncu hakkında ne düşünüyorsun?"></textarea>
                <div class="text-right mt-3"><button class="bg-orange-500 text-white px-8 py-3 rounded-xl font-bold hover:bg-orange-600 transition shadow-lg shadow-orange-500/30">Yorumu Gönder</button></div>
            </form>
            {% else %}
            <div class="bg-indigo-50 p-6 rounded-xl text-indigo-900 font-medium mb-8 border border-indigo-100">Yorum yapmak için <a href="/giris/" class="font-bold underline text-orange-600">giriş yapmalısınız.</a></div>
            {% endif %}

            <div class="space-y-6">
                {% for yorum in sporcu.yorumlar.all %}
                <div class="bg-white p-6 rounded-2xl shadow-sm flex gap-5 border border-gray-50 hover:shadow-md transition">
                    <div class="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-xl shrink-0 shadow-md">{{ yorum.yazan.username.0|upper }}</div>
                    <div>
                        <div class="flex items-center gap-3 mb-2"><span class="font-bold text-gray-900 text-lg">{{ yorum.yazan.username }}</span><span class="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded">{{ yorum.tarih|timesince }} önce</span></div>
                        <p class="text-gray-700 leading-relaxed">{{ yorum.metin }}</p>
                    </div>
                </div>
                {% empty %}
                <p class="text-gray-400 text-center py-8 italic">Henüz yorum yapılmamış.</p>
                {% endfor %}
            </div>
        </div>
    </main>
...
""" + DETAY_HTML_BODY.replace('__NAVBAR__', NAVBAR_HTML).replace('__FOOTER__', FOOTER_HTML)
...
def main():
    print("🚀 HATASIZ FİNAL RESTORASYON BAŞLATILIYOR...\n")
    ...
    print("\n🎉 İŞLEM TAMAM! Hata giderildi. Siteyi yenile.")
