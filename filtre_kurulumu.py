...
# --- 1. VIEWS (Filtreleme Mantığı Eklendi) ---
VIEWS_CODE = """from django.shortcuts import render, get_object_or_404, redirect
from .models import Sporcu, PuanDurumu, Mac, Kulup, MEVKILER, Menajer # MEVKILER'i import ettik
...
from django.db.models import Q # Gelişmiş sorgular için

def anasayfa(request):
    # 1. Tüm sporcuları al
    sporcular = Sporcu.objects.all()
    
    # 2. Filtreleri Uygula (Sorgu Parametreleri: ?isim=zehra&kulup=1...)
    
    # İsim Arama
    isim_ara = request.GET.get('isim')
    if isim_ara:
        sporcular = sporcular.filter(isim__icontains=isim_ara)
    
    # Kulüp Filtresi
    kulup_id = request.GET.get('kulup')
    if kulup_id:
        sporcular = sporcular.filter(kulup_id=kulup_id)
        
    # Mevki Filtresi
    mevki = request.GET.get('mevki')
    if mevki:
        sporcular = sporcular.filter(mevki=mevki)
        
    # Boy Filtresi (Min - Max)
    min_boy = request.GET.get('min_boy')
    max_boy = request.GET.get('max_boy')
    
    if min_boy:
        sporcular = sporcular.filter(boy__gte=min_boy) # gte: Greater Than or Equal (Büyük Eşit)
    if max_boy:
        sporcular = sporcular.filter(boy__lte=max_boy) # lte: Less Than or Equal (Küçük Eşit)

    # 3. Yardımcı Veriler (Select kutuları için)
    tum_kulupler = Kulup.objects.all()
    tum_mevkiler = MEVKILER
    ...
    context = {
        ...
        'maclar': maclar,
        'kulupler': tum_kulupler,
        'mevkiler': tum_mevkiler,
        # Seçili filtreleri şablona geri gönder (form sıfırlanmasın diye)
        'secili_isim': isim_ara,
...
# --- 2. HTML (Filtre Formu Eklendi) ---

# Navbar (Aynen korunuyor)
NAVBAR_HTML = """
...
<head>
    <meta charset="UTF-8">
    <title>VolleyMarkt - Oyuncu Veritabanı</title>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="md:col-span-3">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <span class="text-sm text-gray-500 bg-white px-3 py-1 rounded shadow-sm">{{{{ sporcular|length }}}} Sonuç Bulundu</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {{% empty %}}
                        <div class="col-span-3 text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
                            <p class="text-gray-500 text-lg">Aramanızla eşleşen sporcu bulunamadı.</p>
                            <a href="/" class="text-orange-500 hover:underline text-sm mt-2 block">Filtreleri Temizle</a>
                        </div>
                    {{% endfor %}}
                </div>
            </div>
            ...
            <div class="md:col-span-1 space-y-8">
                ...
                <div class="bg-white rounded-xl shadow p-4">
                    ...
                    <div class="space-y-4">
                        ...
                        {{% empty %}}
                            <p class="text-xs text-gray-400">Maç programı girilmedi.</p>
                        {{% endfor %}}
                    </div>
                </div>
            </div>
...
def main():
    print("🚀 Filtreleme Modülü Yükleniyor...\n")
    write_file('core/views.py', VIEWS_CODE)
    write_file('core/templates/index.html', INDEX_HTML)
    print("\n🎉 İŞLEM TAMAM! 'python manage.py runserver' diyerek test et.")
